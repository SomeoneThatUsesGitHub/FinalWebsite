import { 
  ...
  // Nouveaux imports pour le module d'apprentissage
  learningTopics, type LearningTopic, type InsertLearningTopic,
  learningModules, type LearningModule, type InsertLearningModule,
  learningContents, type LearningContent, type InsertLearningContent,
...
export interface IStorage {
  ...
  getAllLearningTopics(filters?: {category?: string, search?: string, showUnpublished?: boolean}): Promise<LearningTopic[]>;
  getLearningTopicById(id: number): Promise<LearningTopic | undefined>;
  getLearningTopicBySlug(slug: string): Promise<LearningTopic | undefined>;
  createLearningTopic(topic: InsertLearningTopic): Promise<LearningTopic>;
  updateLearningTopic(id: number, data: Partial<InsertLearningTopic>): Promise<LearningTopic | undefined>;
  deleteLearningTopic(id: number): Promise<boolean>;
  incrementTopicPopularity(id: number): Promise<void>;

  // Learning Modules operations
  getLearningModulesByTopicId(topicId: number): Promise<LearningModule[]>;
  ...
  updateLearningModule(id: number, data: Partial<InsertLearningModule>): Promise<LearningModule | undefined>;
  deleteLearningModule(id: number): Promise<boolean>;

  // Learning Contents operations
  getLearningContentsByModuleId(moduleId: number): Promise<LearningContent[]>;
  ...
  updateLearningContent(id: number, data: Partial<InsertLearningContent>): Promise<LearningContent | undefined>;
  deleteLearningContent(id: number): Promise<boolean>;

  // User Learning Progress operations
  getUserLearningProgress(userId: number, topicId?: number): Promise<UserLearningProgress[]>;
  createOrUpdateUserProgress(progress: InsertUserLearningProgress): Promise<UserLearningProgress>;
  markContentAsCompleted(userId: number, contentId: number, score?: number): Promise<UserLearningProgress>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // Learning Topics operations
  async getAllLearningTopics(filters?: {category?: string, search?: string, showUnpublished?: boolean}): Promise<LearningTopic[]> {
    let query = db.select().from(learningTopics);
    
    const conditions = [];
    
    if (filters?.category) {
      conditions.push(eq(learningTopics.category, filters.category));
    }
    
    if (filters?.search) {
      conditions.push(
        or(
          like(learningTopics.title, `%${filters.search}%`),
          like(learningTopics.description, `%${filters.search}%`)
        )
      );
    }
    
    if (!filters?.showUnpublished) {
      conditions.push(eq(learningTopics.isPublished, true));
    }
    ...
    return query.orderBy(desc(learningTopics.popularity), desc(learningTopics.createdAt));
  }
  
  async getLearningTopicById(id: number): Promise<LearningTopic | undefined> {
    const [topic] = await db.select().from(learningTopics).where(eq(learningTopics.id, id));
    return topic;
  }
  
  async getLearningTopicBySlug(slug: string): Promise<LearningTopic | undefined> {
    const [topic] = await db.select().from(learningTopics).where(eq(learningTopics.slug, slug));
    return topic;
  }
  ...
  async updateLearningTopic(id: number, data: Partial<InsertLearningTopic>): Promise<LearningTopic | undefined> {
  ...
  async deleteLearningTopic(id: number): Promise<boolean> {
    const result = await db.delete(learningTopics).where(eq(learningTopics.id, id));
    return result.rowCount > 0;
  }
  
  async incrementTopicPopularity(id: number): Promise<void> {
    await db.update(learningTopics)
      .set({
        popularity: sql`${learningTopics.popularity} + 1`
      })
      .where(eq(learningTopics.id, id));
  }
  
  // Learning Modules operations
  async getLearningModulesByTopicId(topicId: number): Promise<LearningModule[]> {
    return db
      .select()
      .from(learningModules)
      .where(eq(learningModules.topicId, topicId))
  ...
  async createLearningModule(module: InsertLearningModule): Promise<LearningModule> {
    // Vérifier l'ordre maximal actuel
    const [maxOrderResult] = await db
      .select({ maxOrder: sql`MAX(${learningModules.order})` })
      .from(learningModules)
      .where(eq(learningModules.topicId, module.topicId));
    
    // Définir l'ordre comme le max + 1 ou 1 si aucun module n'existe
  ...
  async updateLearningModule(id: number, data: Partial<InsertLearningModule>): Promise<LearningModule | undefined> {
  ...
  async deleteLearningModule(id: number): Promise<boolean> {
    // Supprimer d'abord tous les contenus du module
    await db.delete(learningContents).where(eq(learningContents.moduleId, id));
  ...
  // Learning Contents operations
  async getLearningContentsByModuleId(moduleId: number): Promise<LearningContent[]> {
    return db
      .select()
      .from(learningContents)
      .where(eq(learningContents.moduleId, moduleId))
      .orderBy(learningContents.order);
  }
  
  async getLearningContentById(id: number): Promise<LearningContent | undefined> {
    const [content] = await db.select().from(learningContents).where(eq(learningContents.id, id));
    return content;
  }
  ...
  async createLearningContent(content: InsertLearningContent): Promise<LearningContent> {
    // Vérifier l'ordre maximal actuel
    const [maxOrderResult] = await db
      .select({ maxOrder: sql`MAX(${learningContents.order})` })
      .from(learningContents)
      .where(eq(learningContents.moduleId, content.moduleId));
    
    // Définir l'ordre comme le max + 1 ou 1 si aucun contenu n'existe
    const nextOrder = (maxOrderResult?.maxOrder ?? 0) + 1;
    
    const [newContent] = await db
      .insert(learningContents)
      .values({
        ...content,
  ...
  async updateLearningContent(id: number, data: Partial<InsertLearningContent>): Promise<LearningContent | undefined> {
  ...
  async deleteLearningContent(id: number): Promise<boolean> {
    const result = await db.delete(learningContents).where(eq(learningContents.id, id));
    return result.rowCount > 0;
  }
  
  // User Learning Progress operations
  async getUserLearningProgress(userId: number, topicId?: number): Promise<UserLearningProgress[]> {
    let query = db.select().from(userLearningProgress).where(eq(userLearningProgress.userId, userId));
    
    if (topicId) {
      query = query.where(eq(userLearningProgress.topicId, topicId));
    }
    
    return query.orderBy(desc(userLearningProgress.lastInteraction));
  }
  
  async createOrUpdateUserProgress(progress: InsertUserLearningProgress): Promise<UserLearningProgress> {
    // Vérifier si une entrée existe déjà
    let existingRecord;
    
    if (progress.contentId) {
      // Si on suit un contenu spécifique
      [existingRecord] = await db
        ...
        .where(
          and(
            ...
            eq(userLearningProgress.topicId, progress.topicId),
            eq(userLearningProgress.moduleId, progress.moduleId),
            eq(userLearningProgress.contentId, progress.contentId)
          )
        );
    } else if (progress.moduleId) {
      // Si on suit un module entier
      [existingRecord] = await db
        .select()
        .from(userLearningProgress)
        .where(
          and(
            ...
            eq(userLearningProgress.topicId, progress.topicId),
            eq(userLearningProgress.moduleId, progress.moduleId),
            isNull(userLearningProgress.contentId)
          )
        );
    } else {
      // Si on suit un sujet entier
      [existingRecord] = await db
        .select()
        .from(userLearningProgress)
        .where(
          and(
            eq(userLearningProgress.userId, progress.userId),
            eq(userLearningProgress.topicId, progress.topicId),
            isNull(userLearningProgress.moduleId),
            isNull(userLearningProgress.contentId)
          )
        );
    }
    ...
    if (existingRecord) {
      ...
      const [updatedProgress] = await db
        ...
        .where(eq(userLearningProgress.id, existingRecord.id))
        .returning();
  ...
  async markContentAsCompleted(userId: number, contentId: number, score?: number): Promise<UserLearningProgress> {
    ...
    const [module] = await db
      .select()
      .from(learningModules)
      .where(eq(learningModules.id, content.moduleId));
