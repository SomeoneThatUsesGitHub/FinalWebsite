...
export const learningTopics = pgTable("learning_topics", {
  ...
  description: text("description").notNull(),
  imageUrl: text("image_url"),
  difficulty: text("difficulty").notNull().default("intermédiaire"), // débutant, intermédiaire, avancé
  category: text("category").notNull(), // politique, économie, histoire, institutions, etc.
  authorId: integer("author_id").references(() => users.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  isPublished: boolean("is_published").default(false).notNull(),
  estimatedTime: integer("estimated_time").default(10), // en minutes
  popularity: integer("popularity").default(0), // nombre de fois que le sujet a été commencé
});

export const learningModules = pgTable("learning_modules", {
  id: serial("id").primaryKey(),
  topicId: integer("topic_id").references(() => learningTopics.id, { onDelete: 'cascade' }).notNull(),
  title: text("title").notNull(),
...
export const learningContents = pgTable("learning_contents", {
  ...
  type: text("type").notNull(), // "text", "image", "quiz", "video", "interactive"
  content: json("content").notNull(), // JSON contenant le contenu spécifique au type
  order: integer("order").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const userLearningProgress = pgTable("user_learning_progress", {
  ...
  topicId: integer("topic_id").references(() => learningTopics.id, { onDelete: 'cascade' }).notNull(),
  moduleId: integer("module_id").references(() => learningModules.id, { onDelete: 'cascade' }),
  contentId: integer("content_id").references(() => learningContents.id, { onDelete: 'cascade' }),
  completed: boolean("completed").default(false).notNull(),
  score: integer("score"),
  lastInteraction: timestamp("last_interaction").defaultNow().notNull(),
});

export const insertLearningTopicSchema = createInsertSchema(learningTopics).pick({
  title: true,
  slug: true,
  description: true,
  imageUrl: true,
...
export const insertLearningContentSchema = createInsertSchema(learningContents).pick({
  moduleId: true,
  type: true,
  content: true,
  order: true,
