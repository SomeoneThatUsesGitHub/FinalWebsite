  ...
  const handleAddInstagram = () => {
    ...
    if (url && editor) {
      ...
      if (match && match[2]) {
        ...
        // Extraction des paramètres de l'URL d'origine pour les préserver
        const urlObj = new URL(fullUrl);
        const params = urlObj.search || '';
        
        // Préparer l'URL pour les différents emplacements dans le code Instagram
        const permalinkUrl = `${fullUrl}${params ? '' : '?'}${params ? params.substring(1) + '&' : ''}utm_source=ig_embed&utm_campaign=loading`;
        const linkUrl = permalinkUrl;
        
        // Créer le code d'intégration avec les URLs correctes aux 3 emplacements principaux
        const embedCode = `
          <blockquote class="instagram-media" data-instgrm-captioned data-instgrm-permalink="${permalinkUrl}" data-instgrm-version="14" style=" background:#FFF; border:0; border-radius:3px; box-shadow:0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15); margin: 1px; max-width:540px; min-width:326px; padding:0; width:99.375%; width:-webkit-calc(100% - 2px); width:calc(100% - 2px);">
            <div style="padding:16px;"> 
              <a href="${linkUrl}" style=" background:#FFFFFF; line-height:0; padding:0 0; text-align:center; text-decoration:none; width:100%;" target="_blank"> 
              ...
              <p style=" color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; line-height:17px; margin-bottom:0; margin-top:8px; overflow:hidden; padding:8px 0 7px; text-align:center; text-overflow:ellipsis; white-space:nowrap;">
                <a href="${permalinkUrl}" style=" color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; font-style:normal; font-weight:normal; line-height:17px; text-decoration:none;" target="_blank">Une publication partagée par @politiquensemble</a>
              </p>
            </div>
          </blockquote>
        `;
        ...
        <TabsContent value="editor" className="space-y-4">
          <Card className="p-2 w-full">
            <div className="flex flex-wrap gap-1 p-1 border-b w-full">
              ...
              <Toggle
                onPressedChange={handleAddCarousel}
